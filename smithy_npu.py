...
def compile_rknn(onnx_path: Path, rknn_path: Path) -> Path:
    ...
    try:
        ...
        if ret != 0: raise RuntimeError(f"RKNN config failed: {ret}")
        ret = r.load_onnx(model=str(onnx_path))
        if ret != 0: raise RuntimeError(f"RKNN ONNX load failed: {ret}")
        ret = r.build(do_quantization=False)
        if ret != 0: raise RuntimeError(f"RKNN build failed: {ret}")
        ret = r.export_rknn(str(rknn_path))
        if ret != 0: raise RuntimeError(f"RKNN export failed: {ret}")
    finally:
        r.release()
    return rknn_path
...
def infer_rknn(rknn_path: Path, model_path: Path, repeat: int = 20) -> dict[str, Any]:
    ...
    artifact = json.loads(model_path.read_text())
    features = int(artifact["candidate"]["input_features"])
    # Smithy trains on z-scored observations. A zero vector is the dataset
    # baseline (the learned mean state), avoiding a raw-vs-normalized mismatch.
    x = np.zeros((1, features), dtype=np.float32)
    r = RKNNLite()
    try:
        ret = r.load_rknn(str(rknn_path))
        if ret != 0: raise RuntimeError(f"RKNNLite load failed: {ret}")
        ret = r.init_runtime()
        if ret != 0: raise RuntimeError(f"RKNNLite init_runtime failed: {ret}")
        for _ in range(5): r.inference(inputs=[x])
        timings = []
        ...
        timings.sort()
        evidence = {
            "kind": "smithy_npu_validation", "producer": "smithy", "visibility": "shared",
            "audience": AGENTS, "model_id": artifact.get("model_id"), "rknn": str(rknn_path),
            "input": {"shape": [1, features], "normalization": "dataset_zscore", "sample": "mean_state"},
...
def main() -> int:
    ...
    a = p.parse_args()
    if a.command == "onnx": print(export_onnx(Path(a.model), Path(a.output))); return 0
    if a.command == "compile": print(compile_rknn(Path(a.onnx), Path(a.output))); return 0
    if a.command == "npu": print(json.dumps(infer_rknn(Path(a.rknn), Path(a.model), a.repeat), indent=2)); return 0
    return 1
